...
const VisionProgressChart = dynamic(
  () => import("@/components/vision-progress-chart").then((mod) => mod.VisionProgressChart),
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            {cfs.slice(0, 6).map((cf) => {
              const cfColor = getPlatformColorOption(cf.investment.platform?.color).chartColor;
              return (
              <div
                key={cf.id}
                ...
                style={{
                  ...
                  borderInlineStartColor: cfColor,
                }}
              >
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {formatDate(cf.dueDate, dateLocale)} · {t(`status.pending`)} ·{" "}
      ...
      >
        <div className="space-y-2">
          {invs.slice(0, 6).map((inv) => {
            const platformColor = getPlatformColorOption(inv.platform?.color).chartColor;
            return (
            <Link
              key={inv.id}
              ...
              style={{
                borderInlineStartWidth: 4,
                borderInlineStartColor: platformColor,
              }}
            >
      ...
      <CollapsibleSection id="platform-overview" title={t("dash.platformOverview")}>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {breakdown.length === 0 ? (
            ...
            breakdown.map((b) => {
              const bColor = getPlatformColorOption(b.platformColor).chartColor;
              return (
              <div
                key={b.platformId}
...
const ListSkeleton = React.memo(function ListSkeleton({ rows }: { rows: number }) {
  return (
    <div className="space-y-2">
...
const VisionProgress = React.memo(function VisionProgress({ nav }: { nav: number }) {
...
const ForecastCard = React.memo(function ForecastCard({ label, value }: { label: string; value: number }) {
